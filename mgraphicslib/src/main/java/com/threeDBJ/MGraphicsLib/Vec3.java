...
public class Vec3 implements Serializable {
     ...
     * @see #tmp() **/
    public final static Vec3 tmp = new Vec3();
    /** Static temporary vector. Use with care! Use only when sure other code will not also use this.
     * @see #tmp() **/
    public final static Vec3 tmp2 = new Vec3();
    /** Static temporary vector. Use with care! Use only when sure other code will not also use this.
     * @see #tmp() **/
    public final static Vec3 tmp3 = new Vec3();
     ...
     * @return This vector for chaining */
    public Vec3 mul (Mat4 matrix) {
	float l_mat[] = matrix.val;
	return this.set(x * l_mat[Mat4.M00] + y * l_mat[Mat4.M01] + z * l_mat[Mat4.M02] + l_mat[Mat4.M03], x
			* l_mat[Mat4.M10] + y * l_mat[Mat4.M11] + z * l_mat[Mat4.M12] + l_mat[Mat4.M13], x * l_mat[Mat4.M20] + y
			* l_mat[Mat4.M21] + z * l_mat[Mat4.M22] + l_mat[Mat4.M23]);
    }
     ...
     * @param matrix The matrix.
     * @return This vector for chaining */
    public Vec3 prj (Mat4 matrix) {
	float l_mat[] = matrix.val;
	float l_w = x * l_mat[Mat4.M30] + y * l_mat[Mat4.M31] + z * l_mat[Mat4.M32] + l_mat[Mat4.M33];
	return this.set((x * l_mat[Mat4.M00] + y * l_mat[Mat4.M01] + z * l_mat[Mat4.M02] + l_mat[Mat4.M03]) / l_w, (x
																* l_mat[Mat4.M10] + y * l_mat[Mat4.M11] + z * l_mat[Mat4.M12] + l_mat[Mat4.M13])
			/ l_w, (x * l_mat[Mat4.M20] + y * l_mat[Mat4.M21] + z * l_mat[Mat4.M22] + l_mat[Mat4.M23]) / l_w);
    }
     ...
     * @param matrix The matrix
     * @return This vector for chaining */
    public Vec3 rot (Mat4 matrix) {
	float l_mat[] = matrix.val;
	return this.set(x * l_mat[Mat4.M00] + y * l_mat[Mat4.M01] + z * l_mat[Mat4.M02], x * l_mat[Mat4.M10] + y
			* l_mat[Mat4.M11] + z * l_mat[Mat4.M12], x * l_mat[Mat4.M20] + y * l_mat[Mat4.M21] + z * l_mat[Mat4.M22]);
    }
	...
	Vec3 v2 = target.tmp().sub(x * dot, y * dot, z * dot);
	v2.nor();
    ...
    public boolean epsilonEquals (Vec3 obj, float epsilon) {
	if (obj == null) return false;
     ...
     * @param scalarY
     * @param scalarZ */
    public Vec3 scale (float scalarX, float scalarY, float scalarZ) {
	x *= scalarX;
	y *= scalarY;
	z *= scalarZ;
	return this;
