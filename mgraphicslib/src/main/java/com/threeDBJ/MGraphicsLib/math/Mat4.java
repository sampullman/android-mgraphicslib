 ...
 * <p>
 * <pre>
 * Mat4 mat = new Mat4().trn(position).mul(camera.combined);
 * </pre>
 *
...
public class Mat4 implements Serializable {
     ...
     */
    public Mat4(Quaternion quaternion) {
        this.set(quaternion);
    }
     ...
     * @return This matrix for the purpose of chaining methods together.
     */
    public Mat4 set(Quaternion quaternion) {
     ...
     */
    public Mat4 set(Vec3 xAxis, Vec3 yAxis, Vec3 zAxis, Vec3 pos) {
        val[M00] = xAxis.x;
        val[M01] = xAxis.y;
        val[M02] = xAxis.z;
        val[M10] = yAxis.x;
     ...
     * @return This matrix for the purpose of chaining methods together.
     */
    public Mat4 trn(float x, float y, float z) {
        val[M03] += x;
        val[M13] += y;
        val[M23] += z;
        return this;
     ...
     * @return This matrix for the purpose of chaining methods together.
     */
    public Mat4 setToProjection(float near, float far, float fov, float aspectRatio) {
        idt();
     ...
     * @return This matrix for the purpose of chaining methods together.
     */
    public Mat4 setToOrtho2D(float x, float y, float width, float height) {
     ...
     * @return This matrix for the purpose of chaining methods together.
     */
    public Mat4 setToOrtho2D(float x, float y, float width, float height, float near, float far) {
     ...
     * @return This matrix for the purpose of chaining methods together.
     */
    public Mat4 setToOrtho(float left, float right, float bottom, float top, float near, float far) {
     ...
     * @return This matrix for the purpose of chaining methods together.
     */
    public Mat4 setToTranslation(Vec3 vector) {
        idt();
        val[M03] = vector.x;
        val[M13] = vector.y;
        val[M23] = vector.z;
     ...
     * @return This matrix for the purpose of chaining methods together.
     */
    public Mat4 setToTranslation(float x, float y, float z) {
        idt();
        val[M03] = x;
        val[M13] = y;
        val[M23] = z;
     ...
     * @return This matrix for the purpose of chaining methods together.
     */
    public Mat4 setToTranslationAndScaling(Vec3 translation, Vec3 scaling) {
        idt();
     ...
     * @return This matrix for the purpose of chaining methods together.
     */
    public Mat4 setToTranslationAndScaling(float translationX, float translationY, float translationZ, float scalingX,
     ...
     * @return This matrix for the purpose of chaining methods together.
     */
    public Mat4 setToRotation(Vec3 axis, float angle) {
        idt();
     ...
     * @return This matrix for the purpose of chaining methods together.
     */
    public Mat4 setToRotation(float axisX, float axisY, float axisZ, float angle) {
        idt();
        if (angle == 0) return this;
        return set(quat.set(tmpV.set(axisX, axisY, axisZ), angle));
    }

    static final Vec3 tmpV = new Vec3();
     ...
     * @return This matrix
     */
    public Mat4 setFromEulerAngles(float yaw, float pitch, float roll) {
     ...
     */
    public Mat4 setToScaling(Vec3 vector) {
        idt();
        val[M00] = vector.x;
        val[M11] = vector.y;
        val[M22] = vector.z;
     ...
     * @return This matrix for chaining.
     */
    public Mat4 setToScaling(float x, float y, float z) {
        idt();
        val[M00] = x;
        val[M11] = y;
        val[M22] = z;
    ...
    static Vec3 l_vez = new Vec3();
    static Vec3 l_vex = new Vec3();
    static Vec3 l_vey = new Vec3();
     ...
     * @return This matrix for the purpose of chaining methods together.
     */
    public Mat4 setToLookAt(Vec3 direction, Vec3 up) {
    ...
    static final Mat4 tmpMat = new Mat4();
     ...
     * @return This matrix
     */
    public Mat4 setToLookAt(Vec3 position, Vec3 target, Vec3 up) {
    ...
    static Vec3 right = new Vec3();
    static Vec3 tmpForward = new Vec3();
    static Vec3 tmpUp = new Vec3();

    public Mat4 setToWorld(Vec3 position, Vec3 forward, Vec3 up) {
     ...
     */
    public void lerp(Mat4 matrix, float alpha) {
        for (int i = 0; i < 16; i++)
    ...
    public Mat4 scl(float x, float y, float z) {
        val[M00] *= x;
        val[M11] *= y;
        val[M22] *= z;
        return this;
    ...
    public void getTranslation(Vec3 position) {
        position.x = val[M03];
        position.y = val[M13];
        position.z = val[M23];
    }
    ...
    */
    ...
    static {
        System.loadLibrary("com.threeDBJ.MGraphicsLib.math.Mat4");
    }

    /**
     * Multiplies the matrix mata with matrix matb, storing the result in mata. The arrays are assumed to hold 4x4 column major
     * matrices as you can get from {@link Mat4#val}. This is the same as {@link Mat4#mul(Mat4)}.
     *
     ...
     * from {@link Mat4#val}. The vector array is assumed to hold a 3-component vector, with x being the first element, y being
     ...
     * from {@link Mat4#val}. The vectors array is assumed to hold 3-component vectors. Offset specifies the offset into the
     ...
     * the same as {@link Vec3#mul(Mat4)} applied to multiple vectors.
     *
     * @param mat     the matrix
     ...
     * major matrix as you can get from {@link Mat4#val}. The vector array is assumed to hold a 3-component vector, with x being
     * the first element, y being the second and z being the last component. The result is stored in the vector array. This is the
     * same as {@link Vec3#prj(Mat4)}.
     *
     * @param mat the matrix
     * @param vec the vector.
     */
     ...
     * major matrix as you can get from {@link Mat4#val}. The vectors array is assumed to hold 3-component vectors. Offset
     ...
     * vectors and must be >= 3. This is the same as {@link Vec3#prj(Mat4)} applied to multiple vectors.
     *
     * @param mat     the matrix
     * @param vecs    the vectors
     * @param offset  the offset into the vectors array
     ...
     * column major matrix as you can get from {@link Mat4#val}. The vectors array is assumed to hold 3-component vectors.
     ...
     * subsequent vectors and must be >= 3. This is the same as {@link Vec3#rot(Mat4)} applied to multiple vectors.
     *
     * @param mat     the matrix
     * @param vecs    the vectors
     * @param offset  the offset into the vectors array
     ...
     * @return This matrix for the purpose of chaining methods together.
     */
    public Mat4 rotate(float axisX, float axisY, float axisZ, float angle) {
        if (angle == 0) return this;
     ...
     * @return This matrix for the purpose of chaining methods together.
     */
    public Mat4 rotate(Quaternion rotation) {
