...
public class Quaternion implements Serializable {
    ...
    public Quaternion(Vec3 axis, float angle, boolean inRad) {
        this.setRad(axis, angle);
    }
     ...
     * @return This quaternion for chaining.
     */
    public Quaternion set(Vec3 axis, float angle) {
    ...
    public Quaternion setRad(Vec3 axis, float angle) {
        float l_ang = angle;
        float l_sin = (float) Math.sin(l_ang / 2);
        float l_cos = (float) Math.cos(l_ang / 2);
        return this.set(axis.x * l_sin, axis.y * l_sin, axis.z * l_sin, l_cos).nor();
...
// method
     ...
     * @return This quaternion for chaining.
     */
    public Quaternion setFromAxis(Vec3 axis, float angle) {
...
// final float m11, final float m12, final float m20, final float m21, final float m22
    ...
    public Quaternion setFromMatrix(Mat4 matrix) {
        return setFromAxes(matrix.val[Mat4.M00], matrix.val[Mat4.M01], matrix.val[Mat4.M02], matrix.val[Mat4.M10],
                matrix.val[Mat4.M11], matrix.val[Mat4.M12], matrix.val[Mat4.M20], matrix.val[Mat4.M21],
                matrix.val[Mat4.M22]);
    }
